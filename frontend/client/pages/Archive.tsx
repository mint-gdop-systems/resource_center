    ...
    <div className="space-y-6">
      ...
      <div>
        <Breadcrumb items={breadcrumbItems} />
          ...
          <p className="text-gray-600">Archived files you can bring back anytime</p>
          </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => setShowUnarchiveAll(true)}
            disabled={archivedCount === 0}
            className="inline-flex items-center px-3 py-2 rounded-lg bg-mint-600 text-white text-sm hover:bg-mint-700 disabled:opacity-50 disabled:cursor-not-allowed"
            title="Unarchive all"
          >
          ...
          <button
            ...
            disabled={archivedCount === 0}
            className="inline-flex items-center px-3 py-2 rounded-lg bg-red-600 text-white text-sm hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed"
            title="Delete all"
          >
            Delete All
          </button>
      ...
      {selected.length > 0 && (
        <div className="flex items-center justify-between p-3 bg-mint-50 border border-mint-200 rounded-lg">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center px-3 py-1.5 text-sm text-mint-700 bg-mint-100 hover:bg-mint-200 rounded-md"
            >
              <ArchiveBoxIcon className="h-4 w-4 mr-1" /> Unarchive Selected
            </button>
            <button
              onClick={() => setShowDeleteConfirm({ open: true, ids: selected })}
              className="inline-flex items-center px-3 py-1.5 text-sm text-red-700 bg-red-100 hover:bg-red-200 rounded-md"
            >
              Delete Selected
            </button>
            <button
      ...
      {archivedItems.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200">
          <div className="text-center py-16">
            ...
            <h3 className="mt-4 text-lg font-medium text-gray-900">No archived files</h3>
            <p className="mt-2 text-gray-500 max-w-sm mx-auto">
              Archive old files to keep your workspace organized while preserving important documents.
            </p>
          </div>
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-medium text-gray-700">Archived Files</h3>
            <button
